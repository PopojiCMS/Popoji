		...
		<div>
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb breadcrumb-style1 mg-b-10">
					...
					<li class="breadcrumb-item active" aria-current="page">{{ __('gallery.datatable_list') }}</li>
				</ol>
			</nav>
			<h4 class="mg-b-0 tx-spacing--1">{{ __('gallery.datatable_list') }}</h4>
		</div>
		
		<div>
			<a href="{{ url('dashboard/gallerys/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a>
			<a href="{{ url('dashboard/albums/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('gallery.album') }}</a>
		</div>
	</div>
	
	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/gallerys/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
				<input type="hidden" name="totaldata" id="totaldata" value="0" />
				<table class="table table-striped" id="gallerys-table">
					<thead>
						<tr>
...
<script type="text/javascript">
	$(function() {
		...
		var table = $('#gallerys-table').DataTable({
			processing: true,
			serverSide: true,
			stateSave: true,
			responsive: {
			...
			ajax: '{{ url("dashboard/gallerys/data") }}',
			autoWidth: false,
			order: [[1, 'desc']],
			columnDefs: [{
				targets: 'no-sort',
			...
			columns: [
				...
				{ data: 'id', name: 'gallerys.id' },
				{ data: 'atitle', name: 'albums.title' },
				{ data: 'title', name: 'gallerys.title' },
