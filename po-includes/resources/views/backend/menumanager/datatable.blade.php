...
@section('content')
	<div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-20">
		...
		<div><a href="{{ url('dashboard/menu-manager/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a></div>
	</div>
	
	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/menu-manager/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
				<input type="hidden" name="totaldata" id="totaldata" value="0" />
				<table class="table table-striped" id="menumanager-table">
					<thead>
						<tr>
...
<script type="text/javascript">
	$(function() {
		...
		var table = $('#menumanager-table').DataTable({
			processing: true,
			serverSide: true,
			stateSave: true,
			responsive: {
			...
			ajax: '{{ url("dashboard/menu-manager/data") }}',
			autoWidth: false,
			order: [[1, 'desc']],
			columnDefs: [{
				targets: 'no-sort',
			...
			columns: [
				...
				{ data: 'parent', name: 'menus.parent' },
				{ data: 'title', name: 'menus.title' },
				{ data: 'target', name: 'menus.target' },
