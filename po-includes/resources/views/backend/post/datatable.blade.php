		...
		<div>
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb breadcrumb-style1 mg-b-10">
					...
					<li class="breadcrumb-item active" aria-current="page">{{ __('post.datatable_list') }}</li>
				</ol>
			</nav>
			<h4 class="mg-b-0 tx-spacing--1">{{ __('post.datatable_list') }}</h4>
		</div>
		
		<div><a href="{{ url('dashboard/posts/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a></div>
	</div>
	
	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/posts/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
